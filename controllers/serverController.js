...
class ServerController {
	static getMyoStrength(myo) {
		return (req, res) => {
			...
			if (!connected) {
				return res
					.status(400)
					.json({ meta: { success: false }, error: "Myo not connected" });
			}
			const strength = getStrength(stream);
			res.status(200).json({ meta: { success: true }, data: { strength } });
		};
	}
	...
	static getCustomMetric(myo) {
		return (req, res) => {
			const { connected, interactionAverages, interactionCount } = myo.info;
			if (!connected) {
				return res
					.status(400)
					.json({ meta: { success: false }, error: "Myo not connected" });
